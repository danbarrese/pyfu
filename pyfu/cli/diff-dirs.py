...
parser.add_argument('--path1', '-p1', metavar='P', type=str, nargs=1, dest='path1', required=True,
...
parser.add_argument('--ignore-dates', dest='ignore_dates', action='store_true',
                    help='Ignore file modification dates when comparing files.')
parser.set_defaults(ignore_dates=True)
parser.add_argument('--export-diff', dest='export_diff', action='store_true',
                    help='Export HTML diff of each file to current working directory.')
parser.set_defaults(export_diff=False)
args = parser.parse_args()

output_column_separator = args.column_separator
ignore_file_dates = args.ignore_dates
...
class File:
    ...
    def str(self, indent=""):
        ...
        if self.date_modified is None:
            ...
            ret += time.strftime("%Y.%m.%d %H:%M:%S", self.date_modified).ljust(Folder.longest_date_modified)
        return ret
...
class Folder:
    ...
    def compare_to(self, other_folder):
        ...
        if not self.processed:
            ...
            for file in self.files:
                if file.name in IGNORED_FILE_NAMES:
                    continue
    ...
    def __compare_files_rtl(self, other_folder):
        ...
        for file in self.files:
            ...
            if not file.processed:
                ...
                if export_diff:
                    f1 = 'f1.out'
                    f2 = os.path.abspath(file.full_path)
                    fn = file.full_path.replace('/', '_')
                    subprocess.Popen('vimdiff -R ' + f1 + ' ' + f2 + ' -c TOhtml -c "w! ' + fn + '.html" -c "qa!"', shell=True)
    ...
    def __process_subfolders_rtl(self, other_folder):
        if other_folder.folders is not None:
            for other_subfolder in other_folder.folders:
                if other_subfolder.name in IGNORED_FOLDER_NAMES:
                    continue
    ...
    def __process_subfolders(self, other_folder):
        if self.folders is not None:
            for subfolder in self.folders:
                if subfolder.name in IGNORED_FOLDER_NAMES:
                    continue
                if other_folder is not None:
...
def print_file_diff(folder, file, other_file):
    ...
    if Folder.show_info_with_no_diff or files_are_different:
        ...
        if export_diff:
            f1 = os.path.abspath(file.full_path)
            f2 = os.path.abspath(other_file.full_path)
            fn = file.full_path.replace('/', '_')
            subprocess.Popen('vimdiff -R ' + f1 + ' ' + f2 + ' -c TOhtml -c "w! ' + fn + '.html" -c "qa!"', shell=True)
        print(diffstr)
